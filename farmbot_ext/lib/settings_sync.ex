defmodule Farmbot.SettingsSync do
  ...
  def apply_fbos_map(old_map, new_map) do
    ...
    Map.new(new_map, fn({key, new_value}) ->
      if old_map[key] != new_value do
        Farmbot.Logger.debug 3, "Got new config update: #{key} => #{inspect new_value} old: #{inspect old_map[key]}"
  ...
  when key in @firmware_keys do
    Farmbot.Logger.success 2, "Updating FW param: #{key} => #{inspect val}"
    if val do
  ...
  def do_sync_fw_configs(%{"api_migrated" => true} = api_data) do
    Farmbot.Logger.info 3, "API is the source of truth for Firmware configs. Downloading data."
  ...
  def do_sync_fw_configs(%{"api_migrated" => false} = api_data) do
    ...
    if current["param_version"] do
      Farmbot.Logger.info 3, "Farmbot is the source of truth for Firmware configs. Uploading data."
      payload = Map.put(current, "api_migrated", true) |> Farmbot.JSON.encode!()
      Farmbot.HTTP.delete!("/api/firmware_config")
      Farmbot.HTTP.put!("/api/firmware_config", payload)
    else
      Farmbot.Logger.info 3, "API has never migrated Firmware configs " <>
      "But Farmbot has no valid paramaters. Using defaults."
      payload = Map.put(api_data, "api_migrated", true) |> Farmbot.JSON.encode!()
      Farmbot.HTTP.put!("/api/firmware_config", payload)
  ...
  def do_sync_fw_configs(_) do
    Farmbot.Logger.error 1, "API is out of date. Not syncing firmware settings!"
    :ok
  end
  ...
  def do_sync_fbos_configs(_unimportant_data) do
    Farmbot.Logger.info 3, "Farmbot is the source of truth for Farmbot OS and Firmware configs. Uploading data."
